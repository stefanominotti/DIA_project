...
class PriceUCBLearner(PriceLearner):
    ...
    def update(self, customers):
        for customer in customers:
            ...
            self.expected_reward_per_arm[arm_idx] = (self.expected_reward_per_arm[arm_idx] * (self.samples_per_arm[arm_idx] - 1) + self.rewards_per_arm[arm_idx][-1]) / self.samples_per_arm[arm_idx]
            self.upper_bound_per_arm = self.expected_reward_per_arm + np.sqrt((2 * np.log(self.t) / self.samples_per_arm))

    def get_optimal_arm(self):
        return self.arms[np.argmax(self.expected_reward_per_arm)]